---
title: Using Umami, a free alternative to Google Anlaytics
date: 2022-11-12
tags:
  - azure
	- web
...
I recommend storing the `databasePassword` and `hashSalt` somewhere secure
so that you can login to the database directly. Note that you do _not_ need to
record either to login to Umami.
The sqlserver username is set to `umami`, if you need to change it, edit the
`mysql.bicep` file.
